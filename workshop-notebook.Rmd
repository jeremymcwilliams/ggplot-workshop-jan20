...
# Now take a look at the ggplot cheat sheet. Try changing geom_line() to a different "geom". Any success?


```


Now let's compare multiple countries at once. 
...
ggplot(data=chickwts, mapping=aes(x=feed, y=weight, fill=feed))+geom_boxplot()
...
IrisPlot <- ggplot(Iris_summary, aes(x=Species, y=mean_PL, fill=Species)) + 
                   geom_col() +  
                   geom_errorbar(aes(ymin = mean_PL - sd_PL, ymax = mean_PL + sd_PL), width=0.2)

IrisPlot + labs(y="Petal length (cm) ± s.d.", x = "Species")
